...
<<<<<<< HEAD
    ...
    if text:
        ...
        list_patterns = [
            ...
            r'(?m)^\s*[a-z]\)\s+(.+)$',  # Lettered lists (a), b), etc.)
        ]
        ...
        for pattern in list_patterns:
            ...
            if matches:
                ...
                all_tables.append(list_markdown + '\n')
    
    # Extract lists from the full page text
    if text:
        lists_markdown = extract_lists(text)
        if lists_markdown:
            all_tables.append("**Lists from this page:**" + lists_markdown)
    
    return '\n\n'.join(all_tables) if all_tables else ""

def extract_lists(text):
    """Extract and format lists from text."""
    if not text:
        return ""
    
    # Patterns for different types of lists
    patterns = [
        # Numbered lists (1., 2., etc.)
        (r'(?m)^\s*(\d+[.)]\s+.*(?:\n\s+.*)*)', '1. '),
        # Bullet points (•, -, *)
...
def extract_tables_with_pdfplumber(page):
    ...
    # Extract all text first to identify potential list sections
    full_text = page.extract_text() or ""
    all_tables = []
    seen_tables = set()
    
    # Extract lists from the full page text
    lists_markdown = extract_lists(full_text)
    if lists_markdown:
        all_tables.append("**Lists from this page:**" + lists_markdown)
    ...
    strategies = [
        ...
        # Explicit strategy for tables with explicit borders
        {
            'vertical_strategy': 'lines',
            'horizontal_strategy': 'lines',
            'snap_tolerance': 3,
    ...
    for strategy in strategies:
        try:
            # Extract tables using the current strategy
            extracted_tables = page.extract_tables(strategy)
            ...
            for table in extracted_tables:
                ...
                # Format as markdown and add to results
                if cleaned_table:
                    markdown_table = format_table_as_markdown(cleaned_table)
                    if markdown_table and markdown_table not in seen_tables:
                        extracted_tables.append(markdown_table)
                        seen_tables.add(markdown_table)
                        
        except Exception as e:
            logger.debug(f"Error extracting table with strategy {strategy}: {str(e)}")
            continue
        
        # Add any lists we found in the text
        if lists_markdown and lists_markdown not in seen_tables:
            extracted_tables.append(lists_markdown)
        
        # If we didn't find any tables but have text, try to extract tabular data from text
        if not extracted_tables and full_text:
            # Look for text that looks like a table (e.g., columns separated by multiple spaces)
            table_patterns = [
                r'(?m)^(.{5,50}?)\s{3,}(.{5,50}?)(?:\s{3,}(.{5,50}?))?(?:\s{3,}(.{5,50}?))?(?:\s{3,}(.{5,50}?))?\s*$',
                r'(?m)^(\S.{5,40}?)\s{2,}(\S.{5,40}?)\s{2,}(\S.{5,40}?)\s*$'
            ]
            ...
            for pattern in table_patterns:
                ...
                for match in matches:
                    ...
                    row = [g for g in match.groups() if g and g.strip()]
                    if row and len(row) >= 2:  # At least 2 columns to be considered a table
                        table_rows.append([cell.strip() for cell in row])
                
                if table_rows:
                    # Format as a markdown table
                    markdown_table = format_table_as_markdown(table_rows)
                    if markdown_table and markdown_table not in seen_tables:
                        extracted_tables.append(markdown_table)
                        seen_tables.add(markdown_table)
        
        # Combine all extracted tables and lists with double newlines between them
        result = []
        for table in extracted_tables:
            if table.strip() and table not in result:  # Avoid duplicates
                result.append(table.strip())
        
        if result:
            return '\n\n' + '\n\n'.join(result) + '\n\n'
        # If we still don't have any tables, try to extract any tabular data from the full text
        if full_text and not result:
            # Look for sections with repeating patterns that might be lists or tables
            ...
            # Check each section for tabular or list-like content
            for section in sections:
                # Check if this looks like a list
                list_matches = re.findall(r'(?m)^\s*[•\-*]\s+.+$', section)
                if len(list_matches) >= 3:  # At least 3 list items to be considered a list
                    list_md = '\n'.join(f"- {item.strip('•-* ')}" for item in list_matches)
                    if list_md not in seen_tables:
                        result.append(list_md)
                        seen_tables.add(list_md)
                
                # Check if this looks like a simple table
                rows = [line for line in section.split('\n') if line.strip()]
                if len(rows) >= 3:  # At least 3 rows to be considered a table
                    # Check if rows have similar structure (e.g., similar number of columns)
                    col_counts = [len(re.split(r'\s{2,}', row)) for row in rows]
                    avg_cols = sum(col_counts) / len(col_counts)
                    
                    # If most rows have a similar number of columns, it might be a table
                    if avg_cols >= 2 and max(col_counts) - min(col_counts) <= 2:
                        table_data = []
                        for row in rows:
                            cols = [col.strip() for col in re.split(r'\s{2,}', row) if col.strip()]
                            if cols:
                        ...
                        if table_data:
                            ...
                            if markdown_table and markdown_table not in seen_tables:
                                result.append(markdown_table.strip())
                                seen_tables.add(markdown_table)
        
        # Final result with all extracted content
        if result:
            return '\n\n' + '\n\n'.join(result) + '\n\n'
        return ""
