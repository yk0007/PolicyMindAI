...
def sidebar():
    with st.sidebar:
        ...
        current_provider_display = MODEL_DISPLAY_NAMES.get(
            st.session_state.model_provider, 
            st.session_state.model_provider.value if hasattr(st.session_state.model_provider, 'value') else str(st.session_state.model_provider)
        )
        
        # Find the current provider in the options
        current_provider = next((p for p, d in provider_options if d == current_provider_display), provider_options[0][0])
        
        # Create the selectbox with display names but store the provider enum
        selected_provider_display = st.selectbox(
            "Select Model Provider",
            [d for p, d in provider_options],
            index=[p for p, d in provider_options].index(current_provider) if current_provider in [p for p, d in provider_options] else 0,
        ...
        if st.session_state.get('model_provider') != selected_provider:
